...
"""Huấn luyện các mô hình Random Forest để dự đoán họ topology và loại block.
...
def train(metadata_dir: Path, model_dir: Path, samples_per_template: int, random_seed: int) -> Dict[str, Any]:
    ...
    block_model = RandomForestClassifier(
        n_estimators=200,
        max_depth=10,
        class_weight="balanced",
        random_state=random_seed,
    ...
    joblib.dump(top_model, model_dir / "rf_topology_model.joblib")
    joblib.dump(block_model, model_dir / "rf_block_model.joblib")
