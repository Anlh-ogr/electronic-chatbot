...
if "_bjt_voltage_divider_bias" not in content:
    content = content.replace("    # Solvers cho từng family", bias_code + "\n    # Solvers cho từng family")

import re

# CE
old_ce = r'    def _solve_ce\(self, gain: float, meta: Optional\[Dict\]\) -> SolvedParams:.*?return result'
new_ce = """    def _solve_ce(self, gain: float, meta: Optional[Dict]) -> SolvedParams:
        result = SolvedParams()
        ...
        result.message = f"CE: RC={rc}Ω, RE={re_ext}Ω, R1={r1}Ω, R2={r2}Ω → |Av|≈{actual_gain:.1f} (IC={ic_ma}mA)"
        return result"""
content = re.sub(old_ce, new_ce, content, flags=re.DOTALL)

# CB
old_cb = r'    def _solve_cb\(self, gain: float, meta: Optional\[Dict\]\) -> SolvedParams:.*?return result'
new_cb = """    def _solve_cb(self, gain: float, meta: Optional[Dict]) -> SolvedParams:
        result = SolvedParams()
        hints = (meta or {}).get("solver_hints", {})
        vcc = float((meta or {}).get("vcc", 12.0))
        ic_ma = float(hints.get("ic_ma", 2.0))
        ...
        result.message = f"CB: RC={rc}Ω, RE={re_ext}, R1={r1}, R2={r2} → Av≈{result.actual_gain:.1f} (IC={ic_ma}mA)"
        return result"""
content = re.sub(old_cb, new_cb, content, flags=re.DOTALL)


# CC
old_cc = r'    def _solve_cc\(self, gain: float, meta: Optional\[Dict\]\) -> SolvedParams:.*?return result'
new_cc = """    def _solve_cc(self, gain: float, meta: Optional[Dict]) -> SolvedParams:
        result = SolvedParams()
        hints = (meta or {}).get("solver_hints", {})
        vcc = float((meta or {}).get("vcc", 12.0))
        ic_ma = float(hints.get("ic_ma", 5.0))
        ...
        result.message = f"CC: RE={re_ext}Ω, R1={r1}, R2={r2} → Av≈1"
        return result"""
content = re.sub(old_cc, new_cc, content, flags=re.DOTALL)


# CS
old_cs = r'    def _solve_cs\(self, gain: float, meta: Optional\[Dict\]\) -> SolvedParams:.*?return result'
new_cs = """    def _solve_cs(self, gain: float, meta: Optional[Dict]) -> SolvedParams:
        result = SolvedParams()
        hints = (meta or {}).get("solver_hints", {})
        vcc = float((meta or {}).get("vcc", 12.0))
        ...
        result.message = f"CS: RD={rd}Ω, RS={rs}Ω, R1={rg1}, R2={rg2} → |Av|≈{actual_gain:.1f}"
        return result"""
content = re.sub(old_cs, new_cs, content, flags=re.DOTALL)


# CD
old_cd = r'    def _solve_cd\(self, gain: float, meta: Optional\[Dict\]\) -> SolvedParams:.*?return result'
new_cd = """    def _solve_cd(self, gain: float, meta: Optional[Dict]) -> SolvedParams:
        result = SolvedParams()
        hints = (meta or {}).get("solver_hints", {})
        vcc = float((meta or {}).get("vcc", 12.0))
        ...
        result.gain_formula = "Av ≈ 1"
        result.message = f"CD: RS={rs}Ω, R1={rg1}, R2={rg2} → Av≈1"
        return result"""
content = re.sub(old_cd, new_cd, content, flags=re.DOTALL)

# CG
old_cg = r'    def _solve_cg\(self, gain: float, meta: Optional\[Dict\]\) -> SolvedParams:.*?return result'
new_cg = """    def _solve_cg(self, gain: float, meta: Optional[Dict]) -> SolvedParams:
        result = SolvedParams()
        hints = (meta or {}).get("solver_hints", {})
        vcc = float((meta or {}).get("vcc", 12.0))
        gm_ma = hints.get("gm_ma", 5.0)
        ...
        return result"""
content = re.sub(old_cg, new_cg, content, flags=re.DOTALL)
